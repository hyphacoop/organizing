...
---
# 2020-06-25 Hypha Worker Co-op: Finance
...
- Long check-in about Meet.coop, Toronto Mesh, medical facilities
...
- Year-end **June 30** prep
    - #todo bl: email bylaws to Hema and Rodica and initiate year-end process
        - ask them what we need to do
    - #todo yj: we have fiscal year-end on the 30th, so July 31st we have to file for latest, from May 15 to June 30, yj has access code (in passbolt)
        - answer No to everything
        - enter some numbers from worksheet (bl: QB should spit these out)
        - pay
    - we don't have to involve accountants
- Quickbooks update
    - Receive invoices & match outgoing payments
    - Wagepoint integration -> save us time
    - Multi-currency -> USD bills (save us time)
    - Now we may also be able to integrate TransferWise with QB
    - #todo bl: document on the "upgrade finance tools" issue and bring up at next meeting
- Next year budget
    - ask each WG to budget for next year [ small, medium, big ] (with justification)
    - finance to forecast (revenues - core operation budget), then solicit feedback on loomio
    - propose plans, with revenue milestones, then vote
    - this will happen in parallel with strategic planning by bizdev
    - #todo bl: make issue
